...
@section('content')
    ...
    <h1>{{__('messages.users')}}</h1>
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="{{ route('dashboard.users.index') }}">{{__('messages.users')}}</a></li>
                                ...
                                <div class="tab-content">
                                    ...
                                    @foreach ($models as $index=>$model)
                                        ...
                                        <div class="tab-pane {{ $index == 0 ? 'active' : '' }}" id="{{ $model }}">
                                            ...
                                            @foreach ($maps as $map)
                                                {{--create_users--}}
                                                <label><input type="checkbox" name="permissions[]" {{ $user->hasPermission($map . '_' . $model) ? 'checked' : '' }} value="{{ $map . '_' . $model }}">{{__('messages.' . $map)}}</label>
                                            @endforeach
